...
class ReclorProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, type):
        ...
        for d in lines:
            context = d['argument']
            t = d['topic']            
            l = d['label'] # for test set, there is no label. Just use 0 for convenience.
            s = ''
            '''
            if float(d['stance']) > 0:
                s = 'I agree that'
            else:
                s = 'I do not agree that'
            '''
            ...
            examples.append(
                InputExample(
                    ...
                    topic = t,
                    argument = context,  # this is not efficient but convenient
                    label = l,
                    stance = s
                    )
                )
...
def convert_examples_to_arg_features(
    ...
    for (ex_index, example) in tqdm.tqdm(enumerate(examples), desc="convert examples to features"):
        text_a = example.argument
        text_b = example.topic #example.stance + ' ' + 
        if ex_index % 10000 == 0:
            logger.info("Writing example %d of %d" % (ex_index, len(examples)))
            logger.info('argument: %s', text_a)
            logger.info('topic: %s', text_b)
        stopwords = list(gensim.parsing.preprocessing.STOPWORDS) + punctuations        
        label = float(example.label)
        input = arg_tokenizer(text_a, text_b, tokenizer, stopwords, relations, punctuations, max_ngram, max_length)
        '''
        choices_inputs = []
        ...
        argument_bpe_ids = input["argument_bpe_ids"]
        ...
        domain_bpe_ids = input["domain_bpe_ids"] 
        punct_bpe_ids = input["punct_bpe_ids"]
        features.append(
            InputFeatures(
...
MULTIPLE_CHOICE_TASKS_NUM_LABELS = {"race", 4, "swag", 4, "arc", 4, "syn", 5, "reclor", 1, "logiqa", 4}
