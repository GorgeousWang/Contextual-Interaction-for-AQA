...
def arg_tokenizer(text_a, text_b, tokenizer, stopwords, relations:dict, punctuations:list,
    ...
    def _with_septoken(ngram, tokenizer):
        ...
        #if tokenizer.sep_token in ngram in ngram:    
            flag = True
        else: flag = False
        return flag
    ...
    bpe_tokens = [tokenizer.bos_token] + bpe_tokens_a + [tokenizer.sep_token] + bpe_tokens_b + [tokenizer.eos_token]
    #bpe_tokens = [tokenizer.sep_token] + bpe_tokens_a + [tokenizer.sep_token] + bpe_tokens_b + [tokenizer.sep_token]
    #print(bpe_tokens)
    '''
    bpe_tokens = [tokenizer.bos_token] + bpe_tokens_a + [tokenizer.sep_token] + bpe_tokens_c + [tokenizer.eos_token]
        #+ bpe_tokens_b + [tokenizer.sep_token] + bpe_tokens_c + [tokenizer.eos_token]
    '''
                     

    a_mask = [1] * (len(bpe_tokens_a) + 2) + [0] * (max_length - (len(bpe_tokens_a) + 2))
    
    b_mask = [0] * (len(bpe_tokens_a) + 2) + [1] * (len(bpe_tokens_b) + 1) + [0] * (max_length - len(bpe_tokens_a) - len(bpe_tokens_b) - 3)
    '''
    b_mask = [0] * (len(bpe_tokens_a) + 2) + [1] * (len(bpe_tokens_b) + 1) + [0] * (max_length - len(bpe_tokens_a) - len(bpe_tokens_b) - 3)
    c_mask = [0] * (len(bpe_tokens_a) + 2) + [0] * (len(bpe_tokens_b) + 1) + [1] * (max_length - len(bpe_tokens_a) - len(bpe_tokens_b) - 3)
    '''
